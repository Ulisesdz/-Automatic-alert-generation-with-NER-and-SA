...
def save_model(model: torch.nn.Module, optimizer: optim.Optimizer, epoch: int, model_path: str = "saved_models/best_model.pth"):
    """ Guarda el modelo y el optimizador """
...
def calculate_accuracy_SA(model: torch.nn.Module, dataloader: DataLoader, device: str = 'cpu') -> float:
    ...
    with torch.no_grad():
        for texts, labels, lengths in dataloader:
            texts, labels = texts.to(device), labels.to(device).long()  # Asegurar que labels sean long
            outputs = model(texts, lengths)  # [batch_size, 3]
            
            # Obtener la clase con mayor probabilidad
            preds = torch.argmax(outputs, dim=1)  # Devuelve índices (0,1,2)
...
def train_torch_model(model: torch.nn.Module, train_dataloader: DataLoader,
    ...
    for epoch in range(epochs):
        ...
        for features, labels, text_len in train_dataloader:
            features, labels = features.to(device), labels.to(device).long()  # Asegurar etiquetas long
            optimizer.zero_grad()
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            epochs_no_improve = 0
            save_model(model, optimizer, epoch)  # Guardar el mejor modelo
        else:
            epochs_no_improve += 1
